    ...
    class Program
    {
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            try
            {
                ...
                {
                    m1 = m1 + s;//concat all  strings in first array
                }
                string m2 = "";
                foreach (string s in bulls_string2)
                {
                    m2 = m2 + s;//concat all strings in second array
                }
                if (m1 == m2)//comparing the final strings
                {
                    return true;
                }
                else
            ...
            try
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    else
                    {
                        ...
                        sample[i] = bull_bucks[i];//adding all unique elements to new array
                    }
                }
                return count;
            }
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            ...
            {
                // write your code here.
                int len = Convert.ToInt32(Math.Sqrt(bulls_grid.Length));//get length/width of matrix
                int sum = 0;
                for (int i = 0; i < len; i++)//getting diagnol elements
                {
                    for (int j = 0; j < len; j++)
                    {
                        if (i == j)
                        {
                            sum = sum + bulls_grid[i, j];//First adding all the left diagnol elements
                        }
                        else if (i + j == (len - 1))
                        {
                            sum = sum + bulls_grid[i, j];//Later adding right diagnol elements
                        }
                    }
                }
                return sum;
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                int n = bulls_string6.IndexOf(ch, 0, len);//getting the index of input char
                for (int i = n; i >= 0; i--)
                {
